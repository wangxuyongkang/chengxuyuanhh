<template>
  <div class="sidebar">
    <div class="cate-menu" id="cate-menu">
      <!-- 一级类别 -->
      <h3 v-if="isObject"><a href=""><strong>{{currentCategoryName}}</strong><i id="total_count">商品共{{proNum}}件</i></a>
      </h3>
      <dl>
        <div v-for="item in cateMenu" :key="item.id">
          ...
          <dd v-for="subItem in item.sub_cat" :key="subItem.id">
            <a href="javascript:void(0);" @click="changeMenu(subItem.id)">{{subItem.name}}</a>
          </dd>
        </div>
      </dl>
    </div>
