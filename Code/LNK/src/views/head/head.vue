          ...
          <div class="list" v-show="showShopCar">
            <div class="data">
              <dl v-for="(item, index) in goodsList.goods_list" :key="index">
                <dt><router-link :to="'/app/home/productDetail/'+item.goods.id" target="_blank"><img :src="item.goods.image"></router-link></dt>
                <dd>
...
import {getCategory, getHotSearch, deleteShopCart} from '../../api/api'

export default {
  data () {
    return {
      isShowVip: false, // 会员中心子菜单显示控制，默认不显示
  ...
  methods: {
    ...
    getMenu () { // 获取菜单
      getCategory({}).then(respose => {
        // console.log(respose)
        this.allMenuLabel = respose.data
